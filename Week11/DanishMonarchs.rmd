...
- Out of the four functions, only two of them are tidyverse functions. Regular 'R' only contains the two functions 'read.csv()' and 'read.csv2()'. These functions can clash with other functions in the program and therefore we would rather opt to use the tidyverse functions. The tidyverse functions include 'read_csv()' and 'read_csv2()'.

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
- In this instance the 'class()' function gives insight on the dataset and we see that it is a tibble dataframe.

3. How many columns does the object have when created with these different functions?
- By using the function 'ncol()' we get the exact number of columns. We could also have used 'view()' to get a visual representation of the dataset in 'R'.

4. Show the dataset so that we can see how R interprets each column
- By using 'glimpse()' we can get a short glimpse into how 'R' interprets the dataset.
...
```{r}
kings <- kings %>% 
  mutate(Duration_of_reign = End_year_reign - Start_year_reign)
```
...
```{r}
mean(kings$Duration_of_reign, na.rm = TRUE)
...
```{r}
kings %>% 
  select(Name, Duration_of_reign) %>% 
  filter(Duration_of_reign>20)
...
```{r}
kings %>% 
  arrange(desc(Duration_of_reign)) %>% 
  slice(1:3) %>% 
  mutate(Days_reigned = Duration_of_reign*365) %>% 
  select(Name, Duration_of_reign, Days_reigned)
...
```{r}
kings %>% 
  ggplot(aes(x = Start_year_reign, y = Duration_of_reign)) +
  geom_point() + 
  geom_line() +
  labs(title = "Duration of Reign Through Time", x = "Start of Reign (Year)", y = "Duration (Years)") +
  theme_minimal()
```
